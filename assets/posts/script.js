...
function initMenu() {
  // El script central de /assets/menu ya adiciona manejo de iconos. Limpio duplicados.
  $(".icon-container").off('click').off('hover');

  // Manejar clic en items del menú (solo navegación basada en data-frame).
...
function initGallery() {
  ...
  const groupEls = document.querySelectorAll('#gallery-container .gallery-container');
  groupEls.forEach(groupEl => {
    // Asegurar que cada <a> tenga un data-lg-size correcto (ancho-alto) basado
    // en la imagen real. Muchas entradas venían con un valor hardcodeado
    // (por ejemplo "1600-1067") que puede forzar una animación incorrecta
    // (estirado horizontal en imágenes verticales). Aquí corregimos eso
    // usando naturalWidth/naturalHeight del <img> si está disponible, o
    // precargando la imagen si hace falta.
    groupEl.querySelectorAll('a').forEach(a => {
      try {
        ...
        // No bloquear la inicialización si falla la detección de tamaño
