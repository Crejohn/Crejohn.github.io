...
function handleMainMessage(now) {
  ...
  if (state.initialDelay) {
    if (state.initialDelayTime < state.initialDelayMax) {
      ...
      // Activar showSkip después del delay inicial
      setTimeout(() => {
        state.showSkip = true;
      }, 1000);
    }
...
function renderMessage() {
  // Solo ocultar Skip(S) cuando se muestre el mensaje de última oportunidad o el mensaje del botón azul
  if (state.showLastChance || state.showBluePillMessage) {
    state.showSkip = false;
  } else {
    state.showSkip = true;
  }
  
  if (state.showSkip && !state.contactFormActive) {
    renderSkipOption();
  }
      ...
      <div class="contact-form-container">
        <form id="contactForm">
          <div class="input-container">
            <input type="text" id="name" class="cont" placeholder="Name" required>
            <i class="fa fa-user user"></i>
          </div>
          <div class="input-container">
            <input type="email" id="email" class="cont" placeholder="Email" required>
            <i class="fa fa-envelope email_icon"></i>
          </div>
          <div class="input-container">
            <input type="text" id="subjecting" class="cont" placeholder="Subject" required>
            <i class="fa fa-tag subject"></i>
          </div>
          <div class="textarea-container">
            <textarea id="boxing" class="cont" placeholder="Message" required></textarea>
...
function initMenu() {
  ...
  $('.icon-container').hover(
    function() {
      clearTimeout(menuHideTimeout);
      // Si el cursor está sobre el icono home, ocultar todos los menús
      if ($(this).attr('id') === 'homepic') {
        $(".menu").removeClass("showMenu");
        $(".icon-container").removeClass("scroll-active");
        return;
      }
