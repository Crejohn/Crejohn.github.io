...
class ImageStructureGallery {
  constructor(images, customSlideContent) {
    ...
    // has been opened (not on the main logo_design grid page)
  ...
  buildGalleryFromData() {
    ...
    this.images.forEach((imgObj, idx) => {
      const isVideo = imgObj.isVideo || this.isVideoUrl(imgObj.href || '');
      const thumbUrl = imgObj.thumb || imgObj.poster || (isVideo ? '' : imgObj.href);
      const bgValue = thumbUrl ? 'url(' + thumbUrl + ')' : 'none';

      const div = document.createElement('div');
      div.className = 'grid-item' + (isVideo ? ' media-video' : '');
      div.setAttribute('data-index', idx);
      const inner = document.createElement('div');
      inner.className = 'grid-item-img';
      inner.style.backgroundImage = bgValue;
      div.appendChild(inner);
      this.grid.appendChild(div);

      const thumb = document.createElement('div');
      thumb.className = 'thumbnail' + (idx === this.activeIndex ? ' active' : '') + (isVideo ? ' media-video' : '');
      thumb.setAttribute('data-index', idx);
      const thumbInner = document.createElement('div');
      thumbInner.className = 'thumbnail-img';
      thumbInner.style.backgroundImage = bgValue;
  ...
  // Determine whether a given URL is a video file.
  isVideoUrl(url) {
    return /\.(mp4|webm|ogg|mov)(\?.*)?$/i.test(url);
  }

  // Apply the correct media (image or video) to a slide container.
  // If allowVideo is false, a video URL will be treated as an image placeholder.
  setSlideMedia(element, imgObj, allowVideo = true) {
    if (!element || !imgObj) return;

    // remove any previously-inserted video element so we can swap cleanly
    ...
    // choose what to show as a fallback thumbnail/poster
    const thumb = imgObj.thumb || imgObj.poster;
    const fallbackUrl = thumb || (isVideo ? '' : imgObj.href) || '';

    if (allowVideo && isVideo && imgObj.href) {
      element.style.backgroundImage = fallbackUrl ? `url(${fallbackUrl})` : 'none';
      element.style.backgroundColor = fallbackUrl ? '' : '#000';

      const video = document.createElement('video');
      video.src = imgObj.href;
      video.autoplay = true;
      video.muted = true;
      video.loop = true;
      ...
      // Some browsers require an explicit play() call for autoplay to work reliably.
      const playPromise = video.play();
      if (playPromise && typeof playPromise.then === 'function') {
        playPromise.catch(() => {
          // ignore; autoplay may be blocked but video will still be available if user interacts
        });
      }
    } else {
      element.style.backgroundImage = fallbackUrl ? `url(${fallbackUrl})` : 'none';
  ...
  showSliderView() {
    return new Promise(resolve => {
      try {
        ...
        // apply the appropriate media (image or video) for the active slide
        this.setSlideMedia(this.sliderImage, this.images[this.activeIndex], true);
        // background layers should remain image-based even if the slide is a video
        this.setSlideMedia(this.sliderImageBg, this.images[this.activeIndex], false);
        this.setSlideMedia(this.sliderImageNext, this.images[this.activeIndex], false);
  ...
  transitionToSlide(index) {
    ...
    const newImageObj = this.images[index];

    // prepare transition layers (don't insert <video> into the auxiliary layers)
    this.setSlideMedia(this.sliderImageNext, newImageObj, false);
    this.setSlideMedia(this.sliderImageBg, newImageObj, false);
    ...
    const masterTl = gsap.timeline({
      onComplete: () => {
        // swap main image/video and reset transition layers
        this.setSlideMedia(this.sliderImage, newImageObj, true);
      ...
      try {
        ...
        if (contentEl && groupEl && groupEl.dataset) {
          ...
          if (groupEl.dataset.titleSize) {
            contentEl.style.setProperty('--overlay-title-size', groupEl.dataset.titleSize);
          }
