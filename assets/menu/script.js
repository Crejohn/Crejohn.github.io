$(document).ready(function(){
  ...
  // Close all menus when clicking outside (ignore clicks inside the image-structure overlay)
  $(document).click(function(e){
    // if click originated from an icon or inside the overlay, do nothing
    if ($(e.target).closest('.icon-container').length || $(e.target).closest('.image-structure-overlay').length) {
      return;
    }
...
function Reg(){
  ...
  // Close menu when clicking on top-level menu items
  $(".menu > li").click(function(e){
    e.stopPropagation();
    $(this).parent().removeClass("showMenu");
  });

  // Submenu items should not bubble to parent submenu-trigger items
  $(".menu .submenu li").click(function(e){
    e.stopPropagation();
    // close all menus after selecting a submenu entry
    $(".menu").removeClass("showMenu");
  });
}
