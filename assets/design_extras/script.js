...
document.addEventListener('DOMContentLoaded', function() {
    // Si estamos embebidos, mantenemos el header para que el menú siga accesible.
    ...
    // Inicializar menú flotante en este proyecto (fix: clic en iconos)
    function setupFloatingMenu() {
        ...
        iconContainers.forEach(icon => {
            icon.addEventListener('click', function(e) {
                ...
                const currentlyOpen = document.querySelector('.menu.showMenu');
                if (currentlyOpen && currentlyOpen !== menu) {
                    currentlyOpen.classList.remove('showMenu');
                }
            ...
            icon.addEventListener('mouseleave', function(e) {
                // optional: no immediate hide for better experience
            });
        });

        document.addEventListener('click', function(e) {
            if (!e.target.closest('.icon-container') && !e.target.closest('.menu')) {
                document.querySelectorAll('.menu.showMenu').forEach(openMenu => openMenu.classList.remove('showMenu'));
            }
        });

        // Cerrar menú al seleccionar opción top-level o submenu
        document.querySelectorAll('.menu li').forEach(item => {
            item.addEventListener('click', function(e) {
                e.stopPropagation();
                const menu = this.closest('.menu');
                if (menu) menu.classList.remove('showMenu');
            });
        });
    }
